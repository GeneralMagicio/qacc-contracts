...
interface IBondingCurveBase_v1 {
    ...
    event BuyFeeUpdated(uint256 newBuyFee, uint256 oldBuyFee);
    ...
    event ProjectCollateralFeeWithdrawn(address receiver, uint256 amount);

    /// @notice Event emitted when project collateral fee is added.
    event ProjectCollateralFeeAdded(uint256 amount);
    ...
    event TokensBought(address indexed receiver, uint256 depositAmount, uint256 receivedAmount, address buyer);
    ...
    event ProtocolFeeMinted(address indexed token, address indexed treasury, uint256 feeAmount);
    ...
    function buyFor(address _receiver, uint256 _depositAmount, uint256 _minAmountOut) external;
    ...
    function getStaticPriceForBuying() external view returns (uint256);
    ...
    function calculatePurchaseReturn(uint256 _depositAmount) external returns (uint256 mintAmount);
    ...
    function withdrawProjectCollateralFee(address _receiver, uint256 _amount) external;

    /// @notice Returns the amount of fee in collateral token collected by the project.
    function projectCollateralFeeCollected() external view returns (uint256);
