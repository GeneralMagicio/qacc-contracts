...
interface IRedeemingBondingCurveBase_v1 {
    ...
    event SellFeeUpdated(uint256 newSellFee, uint256 oldSellFee);
    ...
    event TokensSold(address indexed receiver, uint256 depositAmount, uint256 receivedAmount, address seller);
    ...
    function sellTo(address _receiver, uint256 _depositAmount, uint256 _minAmountOut) external;
    ...
    function getStaticPriceForSelling() external view returns (uint256);
    ...
    function calculateSaleReturn(uint256 _depositAmount) external returns (uint256 redeemAmount);
}
